 ...
 */
public class AgeChecker implements MessageListener {
    static final String DEFAULT_FILE = "age.csv";
    private boolean isCheck;

    //TODO: Add your private data member here
    List<TextPair> studentIdAndAgeArrayList = new ArrayList<>();
    ...
    public String onMessageReceived(Message msg) {
        ...
        if (msg.isPrivate() == false && msg.getContent().equals("age")) {
            return "For Private Message only.";
        }

        if (msg.getContent().equals("age") ) {
            isCheck = true;
            return "Please input your ID : ";
        }

        for (int i = 0; i < studentIdAndAgeArrayList.size(); i++) {
            if (msg.getContent().equals(studentIdAndAgeArrayList.get(i).getName()) && isCheck) {
                isCheck =false;
                return "Your age is: " + studentIdAndAgeArrayList.get(i).getValue();
